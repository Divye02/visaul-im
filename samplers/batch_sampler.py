...
def sample_paths(N,
    ...
    if num_cpu == 1:
        return sample_paths_one_core(N, policy, T, env, domain_name, task_name, pegasus_seed, mode)
    else:
        ...
        while sampled_so_far <= N:
            if pegasus_seed is None:
                new_paths = trajectory_sampler.sample_paths_parallel(paths_per_call,
                            policy, T, domain_name, task_name, pegasus_seed, num_cpu, suppress_print=True, mode=mode)
...
def sample_paths_one_core(N,
    ...
    while sampled_so_far < N:
        if mode == 'sample':
            this_path = base_sampler.do_rollout(1, policy, T, env, domain_name, task_name, seed) # do 1 rollout
        elif mode == 'evaluation':
            this_path = eval_sampler.do_evaluation_rollout(1, policy, env, domain_name, task_name, seed)
        else:
