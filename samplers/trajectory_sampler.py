...
def sample_paths(N, policy, T=1e6, env=None, domain_name=None, task_name=None, pegasus_seed=None, mode='sample'):
    if mode == 'sample':
        return base_sampler.do_rollout(N, policy, T, env, domain_name, task_name, pegasus_seed)
    elif mode == 'evaluation':
        return eval_sampler.do_evaluation_rollout(N, policy, env, domain_name, task_name, pegasus_seed)
    else:
...
def sample_paths_parallel(N,
    ...
    if num_cpu == None or num_cpu == 'max':
        ...
        return base_sampler.do_rollout(N, policy, T, None, domain_name, task_name, pegasus_seed)
    else:
