...
from visual_im.utils.make_train_plots import make_train_plots
from visual_im.utils.dm_env import DeepMindEnv
from visual_im.samplers.trajectory_sampler import sample_paths_parallel
import numpy as np
import pickle
...
def train_agent(job_name, agent,
    ...
    mean_pol_perf = 0.0
    e = DeepMindEnv(agent.env.domain_name, agent.env.task_name)
    ...
    for i in range(niter):
        ...
        if evaluation_rollouts is not None and evaluation_rollouts > 0:
            ...
            eval_paths = sample_paths_parallel(N=10, policy=agent.policy, num_cpu=num_cpu,
                                               domain_name=e.domain_name, task_name=e.task_name, mode='evaluation')
